resource "volterra_azure_vnet_site" "site" {
  ...
  resource_group = format("%srg-%s-xc-orchestrated", var.prefix, random_id.id.hex)
...
resource "null_resource" "wait-for-site" {
  ...
  provisioner "local-exec" {
    command = <<-EOF
      ...
      x=1;
      while [ $x -le 60 ]; 
        do VALIDATION_STATE=$(curl -s --location --request GET '${var.volt_api_url}/config/namespaces/system/azure_vnet_sites/${local.site_name}' --header 'Authorization: APIToken ${volterra_api_credential.api.data}'| jq .spec.validation_state); 
...
resource "volterra_tf_params_action" "site" {
  ...
  provisioner "local-exec" {
    command = <<-EOF
      ...
      x=1;
      while [ $x -le 60 ]; 
        do STATE=$(curl -s --location --request GET '${var.volt_api_url}/config/namespaces/system/azure_vnet_sites/${local.site_name}' --header 'Authorization: APIToken ${volterra_api_credential.api.data}'| jq .spec.site_state); 
